...
## Fase 16: Prompt Maestro — Auditoría y Fundamentos de Datos ERP (2026-09-03)

### Motivación
El usuario entregó el archivo `Prompt_Maestro_Antigravity_Nebulae_ERP.md` con instrucciones para convertir los módulos de ventas, compras, tránsito, recepción e inventario en un flujo operativo único, confiable y trazable. El prompt establece un plan de 6 fases de migración.

### Fase 0 — Auditoría y Contrato (sin modificaciones de código)
- Diagnóstico completo del estado real del código. Hallazgos documentados:
  1. Dualidad de modelos: familia legacy (`sales_orders`, `quotations`) vs ERP canónico (`sale_orders`, `customer_requests`). CRM y Finanzas leían de legacy mientras el Hub de Ventas escribía en canónico.
  2. Líneas de SC/COT/VEN/PEC/ENINV como JSON — sin FK ni integridad referencial.
  3. `confirmar_recepcion` con 2 commits dentro de la misma operación y PEC marcado como RECIBIDO aunque sea parcial.
  4. Tracking del PEC como un campo único — imposible modelar múltiples paquetes.
  5. `InventoryLevel` con solo `quantity` — sin dimensiones de estado comercial.
  6. E-Commerce con `stock_disponible` aislado del inventario canónico.
  7. 9 tablas creadas con `CREATE TABLE IF NOT EXISTS` en endpoints de producción (sin Alembic).
- Plan de implementación en 6 fases aprobado y guardado en `implementation_plan.md`.

### Fase 1 — Fundamentos de Datos (commit `1df77ef`)

#### Nuevos Archivos
| Archivo | Propósito |
|---|---|
| `backend/app/models/erp_lines.py` | Modelos normalizados: `CustomerRequestLine`, `SalesQuotationLine`, `SaleOrderLineErp`, `PurchaseOrderLine`, `ProcurementAllocation`, `GoodsReceiptLine`, `InventoryReservation` |
| `backend/app/models/payments.py` | `PaymentTransaction`: pagos atómicos por pedido (ANTICIPO / ABONO / SALDO / DEVOLUCION) |
| `backend/alembic/versions/fa01_erp_fase1.py` | Migración reversible: 8 tablas nuevas + ampliaciones a `inventory_levels`, `goods_receipts`, `customer_requests`, `activity_logs` |
| `backend/scripts/backfill_lines.py` | Script idempotente para migrar datos JSON → líneas normalizadas (ejecutable en producción) |

#### Modificaciones a Archivos Existentes
| Archivo | Cambio |
|---|---|
| `backend/app/api/v1/erp_compras.py` | `confirmar_recepcion` reescrito: atómica (1 sola transacción), idempotente (`idempotency_key`), crea `InventoryOperation` + `InventoryMovement`, deriva estado PEC correctamente (RECIBIDO vs PARCIALMENTE_RECIBIDA), distingue `FISICA` vs `LOGISTICA` |
| `backend/app/models/erp_documents.py` | `GoodsReceipt`: +`idempotency_key`, +`receipt_type`, +`confirmed_by`, +`confirmed_at`. `CustomerRequest`: +`conversation_id`, +`campaign_id`, +`canal_origen`. `ActivityLog`: +`idempotency_key`, +`related_entity_type`, +`related_entity_id` |
| `backend/app/models/inventory.py` | `InventoryLevel`: +`reserved`, +`assigned_customers`, +`in_transit`, +`in_quarantine`, +`qty_nebulae`, +`qty_mau` |
| `backend/app/models/__init__.py` | Registra todos los nuevos modelos para Alembic autogenerate |

#### Nuevas Tablas en Base de Datos
```
customer_request_lines      — líneas SC normalizadas
sales_quotation_lines       — líneas COT normalizadas
sale_order_lines_erp        — líneas PVEN normalizadas
purchase_order_lines        — líneas PEC normalizadas
procurement_allocations     — asignación línea PEC → cliente/stock/Mau
goods_receipt_lines         — líneas ENINV normalizadas
inventory_reservations      — reservas por SKU/bodega con vencimiento
payment_transactions        — pagos atómicos vinculados a pedidos de venta
```

#### Invariantes de Seguridad Establecidos
- Doble llamada a `confirmar_recepcion` con el mismo `idempotency_key` devuelve el resultado anterior sin duplicar stock.
- Un PEC con recepciones parciales queda en `PARCIALMENTE_RECIBIDA`, no en `RECIBIDO`.
- Las recepciones `LOGISTICA` (Miami/Bogotá) no incrementan el stock vendible de Barranquilla.

#### Sin Cambios en el Frontend
Ningún archivo `.tsx` fue modificado. El diseño visual, rutas, componentes y comportamiento del frontend son idénticos.
