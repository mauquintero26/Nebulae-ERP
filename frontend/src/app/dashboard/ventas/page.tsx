...
import {
  ...
  ChevronDown, TrendingUp, ShieldAlert, Filter, LayoutGrid,
  Clock, CheckCircle2, AlertCircle, Zap, ToggleLeft, ToggleRight,
  Save, Plus, Eye
} from 'lucide-react';
...
function ActivitiesPanel({row, onClose}:{row:any, onClose:()=>void}) {
  ...
  const actividades = detail?.actividades || [];
  const chatter     = actividades.filter((a:any)=>a.action==='CHATTER');
  const historial   = actividades.filter((a:any)=>a.action!=='CHATTER');
      ...
      <div className="fixed top-0 right-0 bottom-0 z-50 bg-white shadow-2xl flex flex-col w-[480px] border-l border-slate-200">
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
                ...
                <div className="space-y-2">
                  {chatter.map((a:any,i:number)=>(
                    <div key={i} className="bg-green-50 border border-green-100 rounded-xl p-3">
                      ...
                      <p className="text-xs text-green-600 mt-1">{a.user_name} - {fDate(a.created_at)} {fTime(a.created_at)}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Historial */}
            <div>
              <p className="text-xs font-black text-slate-400 uppercase mb-3 flex items-center gap-1"><Activity size={12}/> Historial de Actividad</p>
              {historial.length === 0 ? (
                ...
                <div className="space-y-3 relative before:absolute before:left-2 before:top-0 before:bottom-0 before:w-0.5 before:bg-slate-100">
                  {[...historial].reverse().map((a:any,i:number)=>(
                    <div key={i} className="flex gap-3 pl-7 relative">
                      <div className="absolute left-0 top-2 w-4 h-4 rounded-full border-2 border-white flex-shrink-0"
...
function ConfigPanel({alertDias, onSave, onClose}:{alertDias:any, onSave:(cfg:any)=>void, onClose:()=>void}) {
  ...
  async function save() {
    setSaving(true);
    try {
      await apiFetch('/ventas/config', {method:'PATCH', body:JSON.stringify(cfg)});
      onSave(cfg);
      setSaved(true);
      ...
      <div className="fixed top-0 right-0 bottom-0 z-[70] bg-white shadow-2xl flex flex-col w-[520px] border-l border-slate-200">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-slate-800 text-lg">Configuracion Ventas Hub</h2>
              <p className="text-xs text-slate-500">Automatizaciones, alertas y preferencias del modulo</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-xl text-slate-500"><X size={18}/></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-black text-amber-800 text-sm uppercase tracking-wide">Alertas de Tiempo Sin Atender</h3>
            </div>
            <p className="text-xs text-amber-700 mb-4">Si un documento pasa X dias sin cambio de estado, aparece alerta roja en el banner y en CRM Seguimientos.</p>
            <div className="space-y-3">
              {[
                {k:'alerta_sc_dias',  label:'Solicitudes de Cliente (SC)'},
                {k:'alerta_cot_dias', label:'Cotizaciones (COT)'},
                {k:'alerta_ven_dias', label:'Pedidos de Venta (PVEN)'},
              ].map(({k,label})=>(
                <div key={k} className="flex items-center justify-between bg-white rounded-xl p-3 border border-amber-100">
                  <span className="text-sm font-bold text-slate-700">{label}</span>
                  <div className="flex items-center gap-2">
                    <input type="number" min="1" max="30" value={(cfg as any)[k]}
          ...
          <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-black text-indigo-800 text-sm uppercase tracking-wide">Automatizaciones</h3>
            </div>
            <div className="space-y-3">
              <Toggle k="auto_lead_crm"
                label="Auto-crear Lead en CRM"
                desc="Al crear una SC con cliente registrado, crea automaticamente un Lead en el pipeline CRM."/>
              <Toggle k="auto_chatter_ia"
                label="Chatter asistido por IA"
                desc="Sugiere respuestas en el Chatter basado en el historial del cliente y la solicitud."/>
              <Toggle k="notif_whatsapp"
                label="Notificacion WhatsApp automatica"
                desc="Al cambiar estado PVEN a LISTO ENTREGA, envia mensaje automatico al cliente por WhatsApp."/>
            </div>
          </div>

          {/* Flujo pipeline info */}
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
            <h3 className="font-black text-slate-600 text-xs uppercase tracking-wide mb-3 flex items-center gap-2">
              <Activity size={13}/> Flujo del Pipeline de Ventas
            </h3>
            <div className="flex items-center gap-1.5 flex-wrap text-xs font-bold">
              {[
                ...
                <React.Fragment key={n}>
                  <span className={`bg-${c}-100 text-${c}-700 border border-${c}-200 px-3 py-1.5 rounded-lg`}>{n}</span>
                  {i<a.length-1 && <span className="text-slate-300 text-base">→</span>}
                </React.Fragment>
              ))}
            </div>
            <p className="text-xs text-slate-400 mt-3">Cada transicion registra actividad y avanza el Lead en CRM pipeline.</p>
          </div>
        </div>
...
export default function VentasHub() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const scList  = (Array.isArray(sc)  ? sc  : (sc?.items||sc?.data||[])).map((x:any)=>({...x,tipo:'SC'}));
      const cotList = (Array.isArray(cot) ? cot : (cot?.items||cot?.data||[])).map((x:any)=>({...x,tipo:'COT'}));
      const venList = (Array.isArray(ven) ? ven : (ven?.items||ven?.data||[])).map((x:any)=>({...x,tipo:'VEN'}));
  ...
  const kpis = [
    {
      ...
      color:'indigo',
      sub: scAtrasadas.length>0 ? `${scAtrasadas.length} sin atender` : 'Todas atendidas',
    ...
    },
    {
      ...
      color:'amber',
      sub: cotAtrasadas.length>0 ? `${cotAtrasadas.length} sin atender` : 'Todas atendidas',
    ...
    },
    {
      ...
      color:'emerald',
      sub: venAtrasadas.length>0 ? `${venAtrasadas.length} sin atender` : 'Al dia',
      ok: venAtrasadas.length===0,
    },
    {
      label:'Facturado (total)',
      value: fCOP(pedidos.filter(p=>p.estado==='FACTURADO').reduce((s:number,p:any)=>s+(p.total_cop||p.total||0),0)),
      color:'purple',
      sub: `${pedidos.filter(p=>p.estado==='FACTURADO').length} pedidos`,
      ok: true,
    },
  ];
  ...
  async function handleBulkDelete() {
    if(!selectedIds.size||!confirm('Cancelar/rechazar los seleccionados?')) return;
    for(const idStr of Array.from(selectedIds)) {
      const [tipo,id]=idStr.split('|');
      const path=tipo==='SC'?`/ventas/solicitudes/${id}`:tipo==='COT'?`/ventas/cotizaciones/${id}`:`/ventas/pedidos/${id}`;
  ...
  async function handleAskAI() {
    ...
    } catch { setAiResponse('El asistente IA de ventas esta en configuracion. Pronto estara disponible.'); }
    setAiLoading(false);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {showConfig && (
        <ConfigPanel
          alertDias={alertDias}
          onSave={cfg=>{setAlertDias(cfg);setShowConfig(false);loadData();}}
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-2 overflow-x-auto flex items-center gap-2 shadow-sm sticky top-0 z-30">
        <span className="text-xs font-black text-slate-400 uppercase tracking-wider mr-3 shrink-0">Ventas:</span>
        {SUB_MODULES.map(m=>(
          <Link key={m.name} href={m.path}
            className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-bold border transition-colors ${pathname===m.path?'bg-indigo-600 text-white border-indigo-600':'text-slate-600 hover:bg-indigo-50 hover:text-indigo-700 border-transparent hover:border-indigo-200'}`}>
            {m.name}
          </Link>
        ))}
      </div>
      ...
      {totalAtrasadas > 0 && (
        <div className="bg-red-50 border-b border-red-200 px-6 py-2.5 flex items-center gap-3">
          ...
          <button onClick={()=>setShowConfig(true)} className="text-xs text-red-600 hover:text-red-800 font-bold border border-red-300 px-2.5 py-1 rounded-lg">Config</button>
          <button onClick={loadData} className="text-xs bg-red-100 hover:bg-red-200 text-red-700 px-3 py-1.5 rounded-lg font-bold shrink-0 flex items-center gap-1">
            <RefreshCw size={11}/> Actualizar
          </button>
        </div>
      )}

      <div className="p-5 flex-1 flex flex-col max-w-[1600px] mx-auto w-full space-y-5">

        {/* ── KPI Cards ── */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {kpis.map((k,i)=>(
            <div key={i} className={`bg-white rounded-2xl p-5 border shadow-sm hover:shadow-md transition-shadow ${k.ok?'border-slate-200':'border-red-200'}`}>
              <p className="text-xs font-black text-slate-400 uppercase tracking-wide mb-2">{k.label}</p>
              <p className="text-3xl font-black text-slate-800">{k.value}</p>
              <p className={`text-xs font-bold mt-1.5 flex items-center gap-1 ${k.ok?'text-emerald-600':'text-red-500'}`}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex bg-white rounded-xl shadow-sm p-1 border border-slate-200">
            {['Todos','SC','Cotizaciones','Pedidos de Venta','Analisis'].map(tab=>(
              <button key={tab} onClick={()=>{setActiveTab(tab);setSelectedIds(new Set());}}
                className={`px-3.5 py-2 rounded-lg text-sm font-bold transition-all ${activeTab===tab?'bg-indigo-600 text-white shadow':'text-slate-600 hover:bg-slate-50'}`}>
                {tab}
              </button>
            ))}
          </div>
          ...
          {activeTab !== 'Analisis' && (
            <>
              {/* Search */}
              <div className="flex items-center bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-sm gap-2 flex-1 min-w-[220px] max-w-[340px]">
                <Search size={15} className="text-slate-400 shrink-0"/>
                <input value={search} onChange={e=>setSearch(e.target.value)}
                  placeholder="Buscar numero, cliente, asesor..."
              ...
              <div className="relative">
                <button onClick={()=>setShowFilters(f=>!f)}
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-sm font-bold shadow-sm bg-white transition-colors ${filterEstado?'border-indigo-400 text-indigo-700 bg-indigo-50':'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                  <Filter size={13}/> Filtro
                  {filterEstado&&<span className="bg-indigo-600 text-white text-[10px] px-1.5 py-0.5 rounded-full">1</span>}
                </button>
                {showFilters && (
                  <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-2xl shadow-2xl p-4 z-20 w-60">
                    <p className="text-xs font-black text-slate-400 uppercase mb-2">Filtrar por Estado</p>
                    <select value={filterEstado} onChange={e=>setFilterEstado(e.target.value)}
                      className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-sm outline-none mb-3">
                    ...
                    <button onClick={()=>{setFilterEstado('');setShowFilters(false);}}
                      className="text-xs text-slate-500 hover:text-red-600 font-bold w-full text-center">
                      Limpiar filtro
                    </button>
                  </div>
                )}
              ...
              <div className="flex items-center bg-white border border-slate-200 rounded-xl shadow-sm p-1">
                <button onClick={()=>setViewMode('lista')} className={`p-2 rounded-lg transition-colors ${viewMode==='lista'?'bg-indigo-100 text-indigo-700':'text-slate-500 hover:bg-slate-50'}`} title="Vista Lista"><List size={17}/></button>
                <button onClick={()=>setViewMode('kanban')} className={`p-2 rounded-lg transition-colors ${viewMode==='kanban'?'bg-indigo-100 text-indigo-700':'text-slate-500 hover:bg-slate-50'}`} title="Vista Kanban"><LayoutGrid size={17}/></button>
              </div>
            </>
          )}

          <div className="ml-auto flex items-center gap-2">
            <button onClick={loadData} title="Actualizar" className="p-2 bg-white border border-slate-200 rounded-xl shadow-sm text-slate-500 hover:bg-slate-50 transition-colors">
              <RefreshCw size={15} className={loading?'animate-spin':''}/>
            </button>
            <button onClick={()=>setShowConfig(true)}
              className="flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 rounded-xl shadow-sm text-sm font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-200 transition-colors">
              <Settings size={14}/> Config
            </button>
            {totalAtrasadas>0 && (
              <div className="flex items-center gap-1 px-2.5 py-2 bg-red-50 border border-red-200 rounded-xl text-xs font-bold text-red-700">
                <Bell size={12}/> {totalAtrasadas}
              </div>
            )}
          </div>
        </div>
        ...
        /* ============ ANALYTICS TAB ============ */
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-2 items-center bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
              <span className="text-xs font-black text-slate-400 uppercase">Periodo:</span>
              {[{k:'7d',l:'7 dias'},{k:'30d',l:'1 mes'},{k:'90d',l:'Trimestre'},{k:'180d',l:'Semestre'},{k:'1y',l:'Anual'}].map(({k,l})=>(
                <button key={k} onClick={()=>setAnalyticsRange(k)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-bold ${analyticsRange===k?'bg-indigo-600 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{l}</button>
              ))}
              <div className="h-6 w-px bg-slate-200 mx-1"/>
              <input type="text" placeholder="Filtrar producto..." value={analyticsProduct}
                onChange={e=>setAnalyticsProduct(e.target.value)}
                className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-indigo-200"/>
              <select value={analyticsTopN} onChange={e=>setAnalyticsTopN(e.target.value)}
                className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm outline-none">
                <option value="5">Top 5</option>
                <option value="10">Top 10</option>
                <option value="25">Top 25</option>
              </select>
              <button onClick={loadAnalytics} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1">
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                {label:'Total Ventas COP', value:fCOP(analytics?.total_ventas||0)},
                {label:'Ticket Promedio',  value:fCOP(analytics?.ticket_promedio||0)},
                {label:'Conversion SC-COT',value:`${analytics?.conv_sc_cot||0}%`},
                {label:'Conversion COT-VEN',value:`${analytics?.conv_cot_ven||0}%`},
              ].map((k,i)=>(
                <div key={i} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                  <p className="text-slate-500 text-xs font-bold uppercase mb-2">{k.label}</p>
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h3 className="font-black text-slate-800 mb-4 flex items-center gap-2"><TrendingUp size={16} className="text-indigo-600"/> Ventas por Dia</h3>
                  ...
                  {(analytics?.ventas_por_dia||[]).map((d:any,i:number)=>{
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <div className="w-20 text-xs text-slate-500 shrink-0">{d.fecha}</div>
                        <div className="flex-1 bg-slate-100 rounded-full h-4 overflow-hidden">
                          <div className="bg-indigo-500 h-full rounded-full transition-all" style={{width:`${(d.total/max)*100}%`}}/>
                        </div>
                        <div className="w-24 text-right text-xs font-bold text-slate-700">{fCOP(d.total)}</div>
                      </div>
                    );
                  })}
                  {!(analytics?.ventas_por_dia?.length) && <p className="text-slate-400 text-sm text-center py-8">Sin datos para el periodo seleccionado</p>}
                </div>
              </div>

              {/* Top clientes */}
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h3 className="font-black text-slate-800 mb-3">Top Clientes</h3>
                <div className="flex gap-2 mb-3">
                  <span className="text-xs bg-amber-50 border border-amber-200 text-amber-700 px-2 py-1 rounded-full font-bold">
                    &gt;$500k: {(analytics?.top_clientes||[]).filter((c:any)=>c.total>=500000).length}
                  </span>
                  <span className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-1 rounded-full font-bold">
                    &gt;$1M: {(analytics?.top_clientes||[]).filter((c:any)=>c.total>=1000000).length}
                  </span>
                </div>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-left text-xs text-slate-400 font-black uppercase">
                      <th className="pb-2">#</th><th>Nombre</th><th className="text-right">Total COP</th><th className="text-right">Seg.</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(analytics?.top_clientes||[]).map((c:any,i:number)=>(
                      <tr key={i} className="border-b last:border-0 hover:bg-slate-50">
                        ...
                        <td className="py-2 text-right">
                          {c.total>=1000000?<span className="text-xs bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded font-bold">&gt;1M</span>
                            :c.total>=500000?<span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-bold">&gt;500k</span>:null}
                        </td>
                      </tr>
                    ))}
                    {!(analytics?.top_clientes?.length) && <tr><td colSpan={4} className="text-center py-6 text-slate-400 text-xs">Sin datos</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="font-black text-slate-800 mb-4 flex items-center gap-2">
                <MessageSquare size={18} className="text-indigo-600"/> AI Sales Assistant
              </h3>
              <div className="flex gap-2 mb-4">
                <input value={aiQuestion} onChange={e=>setAiQuestion(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleAskAI()}
                  placeholder="Ej: Cual fue el cliente que mas compro este mes? Que productos vendemos mas?"
              ...
              {aiLoading && <div className="text-slate-500 animate-pulse text-sm py-2">Analizando ventas...</div>}
              {aiResponse && (
                <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl text-slate-700 text-sm whitespace-pre-wrap">{aiResponse}</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-slate-50 text-slate-400 font-black text-xs uppercase tracking-wide border-b border-slate-200">
                  <tr>
                    <th className="px-5 py-3 w-10">
                      <input type="checkbox" className="rounded border-slate-300" onChange={e=>{
                        if(e.target.checked) setSelectedIds(new Set(filteredData.map((d:any)=>`${d.tipo}|${d.id}`)));
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredData.length===0 && (
                    <tr>
                      <td colSpan={10} className="text-center py-16 text-slate-400">
                        ...
                        <p className="font-medium">Sin resultados{search ? ` para "${search}"` : ''}</p>
                      </td>
                    </tr>
                  )}
                  {filteredData.map((row:any,idx:number)=>{
                    ...
                    const isAtrasada=
                      (row.tipo==='SC'  && scAtrasadas.some((x:any)=>x.id===row.id))  ||
                      (row.tipo==='COT' && cotAtrasadas.some((x:any)=>x.id===row.id)) ||
                      (row.tipo==='VEN' && venAtrasadas.some((x:any)=>x.id===row.id));
                    return (
                      <tr key={idx}
                        className={`hover:bg-indigo-50/40 cursor-pointer group transition-colors ${isAtrasada?'border-l-4 border-l-red-400 bg-red-50/30':''}`}
                        onClick={e=>{
                          if((e.target as any).closest('input')||(e.target as any).closest('button')) return;
                          if(row.tipo==='VEN') openVenDetail(row.id);
                          else window.location.href=row.tipo==='SC'?`/dashboard/ventas/solicitud`:`/dashboard/ventas/cotizacion`;
                        }}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            {isAtrasada && <AlertCircle size={12} className="text-red-500" title={`Sin atender mas de ${row.tipo==='SC'?scAlertDias:row.tipo==='COT'?cotAlertDias:venAlertDias} dias`}/>}
                          </div>
                        </td>
                        <td className="px-4 py-3 font-bold text-slate-800">{row.numero}</td>
                        <td className="px-4 py-3 text-slate-700">{row.customer_name||row.cliente?.nombre||'-'}</td>
                        <td className="px-4 py-3 text-slate-400 text-xs">{row.advisor_name||row.asesor?.nombre||'-'}</td>
                        <td className="px-4 py-3 text-slate-400 text-xs">{fDate(row.created_at||row.fecha)}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getEstadoClass(row.estado)}`}>{row.estado}</span>
                        </td>
                        <td className="px-4 py-3 text-right font-bold text-slate-800">{fCOP(row.total_cop||row.total||row.monto||0)}</td>
                        {/* Activities column — shows on hover */}
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={e=>{e.stopPropagation();setActRow(row);}}
                            className="opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1.5 mx-auto px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 rounded-lg text-xs font-bold">
                            <Activity size={11}/> Ver
                          </button>
                        </td>
                        {/* 3-dot menu */}
                        <td className="px-4 py-3 relative group/m">
                          <button className="text-slate-300 hover:text-slate-600 transition-colors group-hover:text-slate-400">
                            <MoreHorizontal size={18}/>
                          </button>
                          <div className="absolute right-10 top-1 bg-white shadow-2xl rounded-2xl border border-slate-100 py-2 w-52 invisible group-hover/m:visible z-20">
                            <button className="w-full text-left px-4 py-2 hover:bg-indigo-50 text-sm font-medium flex items-center gap-2 text-indigo-700"
                              onClick={e=>{e.stopPropagation();setActRow(row);}}>
                              <Activity size={13}/> Ver Actividad
                            </button>
                            <button className="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm font-medium"
                              onClick={e=>{e.stopPropagation();if(row.tipo==='VEN')openVenDetail(row.id);else window.location.href=row.tipo==='SC'?`/dashboard/ventas/solicitud`:`/dashboard/ventas/cotizacion`;}}>
                              Ver Detalle
                            </button>
                            <button className="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm font-medium"
                              onClick={e=>{e.stopPropagation();window.location.href=row.tipo==='SC'?`/dashboard/ventas/solicitud`:`/dashboard/ventas/cotizacion`;}}>
                              Editar
                            </button>
                            <div className="border-t border-slate-100 my-1"/>
                            <button className="w-full text-left px-4 py-2 hover:bg-red-50 text-sm font-medium text-red-600"
            ...
            <div className="px-5 py-2.5 border-t border-slate-100 text-xs text-slate-400 font-medium flex items-center justify-between">
              <span>{filteredData.length} registros{search&&` (filtrado)`}</span>
        ...
        /* ============ KANBAN VIEW ============ */
          ...
          <div className="flex-1 flex gap-4 overflow-x-auto pb-4">
            {['Pendiente','En Proceso','Completado','Cancelado'].map(col=>(
              <div key={col} className="w-80 flex-shrink-0 flex flex-col bg-white border border-slate-200 rounded-2xl shadow-sm"
                onDragOver={e=>e.preventDefault()} onDrop={e=>handleDrop(e,col)}>
                <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                  <h3 className="font-black text-slate-700">{col}</h3>
                ...
                <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2 min-h-[200px]">
                  {filteredData.filter(d=>getKanbanCol(d)===col).map((row:any,i:number)=>{
                    ...
                    return (
                      <div key={i} draggable
                        onDragStart={e=>handleDragStart(e,`${row.tipo}|${row.id}`)}
                        onClick={()=>{if(row.tipo==='VEN')openVenDetail(row.id);}}
                        className={`bg-white p-4 rounded-xl shadow-sm border cursor-grab active:cursor-grabbing hover:border-indigo-300 transition-colors group ${isAtrasada?'border-red-300 bg-red-50/40':'border-slate-200'}`}>
                        ...
                        <p className="text-xs text-slate-500 mb-2 truncate">{row.customer_name||row.cliente?.nombre||'Cliente'}</p>
                        <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                          ...
                          <button onClick={e=>{e.stopPropagation();setActRow(row);}}
                            className="opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1 px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-bold border border-indigo-200">
                  ...
                  {filteredData.filter(d=>getKanbanCol(d)===col).length===0 && (
                    <p className="text-center text-xs text-slate-300 font-medium py-8">Arrastra aqui</p>
                  )}
                </div>
              </div>
            ))}
        ...
        {selectedIds.size > 0 && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 z-40 border border-slate-700">
            ...
            <select onChange={e=>handleBulkChangeEstado(e.target.value)} defaultValue=""
              className="bg-slate-800 border border-slate-600 text-white text-sm rounded-xl px-3 py-1.5 outline-none">
            ...
            <button onClick={()=>setSelectedIds(new Set())} className="p-2 text-slate-400 hover:text-white"><X size={16}/></button>
          </div>
        )}
        ...
        {selectedVenId && (
          <>
            <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-40" onClick={()=>setSelectedVenId(null)}/>
            <div className="fixed top-0 bottom-0 right-0 z-50 bg-white shadow-2xl flex flex-col" style={{left:'240px'}}>
              <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 bg-slate-50">
                ...
                <button onClick={()=>setSelectedVenId(null)} className="p-2 hover:bg-slate-200 rounded-xl text-slate-500"><X size={22}/></button>
              </div>
              {!venDetail ? (
                ...
                <div className="flex-1 flex overflow-hidden">
                  <div className="w-[45%] border-r border-slate-100 bg-slate-50/50 p-7 overflow-y-auto space-y-4">
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                      <p className="text-xs font-black text-slate-400 uppercase mb-2">Cliente</p>
                      <p className="font-bold text-lg">{venDetail.customer_name||venDetail.cliente?.nombre||'-'}</p>
                      <div className="grid grid-cols-2 gap-3 mt-3 text-sm">
                        <div><span className="text-xs text-slate-400 block mb-0.5">Fecha Estimada</span><span className="font-medium">{fDate(venDetail.fecha_entrega_estimada||venDetail.fecha_entrega)}</span></div>
                        <div><span className="text-xs text-slate-400 block mb-0.5">COT Origen</span><span className="font-medium text-indigo-600">{venDetail.cot_numero||'-'}</span></div>
                      </div>
                    </div>
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                      <p className="text-xs font-black text-slate-400 uppercase mb-3">Montos</p>
                      {[
                        ['Total COP', venDetail.total_cop||venDetail.total],
                        ['Anticipo', venDetail.anticipo_cop||venDetail.anticipo],
                        ['Saldo', venDetail.saldo_cop||(venDetail.total_cop-(venDetail.anticipo_cop||0))],
                      ].map(([label,val]:any,i:number)=>(
                        <div key={i} className="flex justify-between py-2 border-b last:border-0">
                          <span className="text-slate-500 text-sm">{label}</span>
                          <span className={`font-bold text-sm ${label==='Anticipo'?'text-emerald-600':label==='Saldo'?'text-indigo-600':''}`}>{fCOP(val)}</span>
                        </div>
                      ))}
                    </div>
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                      <p className="text-xs font-black text-slate-400 uppercase mb-3">Productos</p>
                      {(venDetail.productos||[]).map((p:any,i:number)=>(
                        <div key={i} className="flex justify-between p-2 bg-slate-50 rounded-lg mb-2 text-sm">
                          <span className="font-medium text-slate-700">{p.product_name||p.nombre}</span>
                          <span className="font-bold">{fCOP(p.qty*(p.unit_price_cop||p.precio_unitario||0))}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="w-[55%] p-7 overflow-y-auto bg-white">
                    <p className="text-xs font-black text-slate-400 uppercase mb-3">Cambiar Estado</p>
                    <div className="flex flex-wrap gap-2 mb-6 p-4 bg-slate-50 rounded-2xl">
                      {['PENDIENTE_COMPRA','EN_PROCESO','LISTO_ENTREGA','ENTREGADO','FACTURADO','CANCELADO'].map(est=>(
                        <button key={est} onClick={async()=>{
                          try{await apiFetch(`/ventas/pedidos/${venDetail.id}`,{method:'PATCH',body:JSON.stringify({estado:est})});openVenDetail(venDetail.id);loadData();}
                          catch(e:any){setToast({msg:e.message,type:'error'});}
                        }} className={`px-3 py-2 rounded-xl text-xs font-bold border transition-colors ${venDetail.estado===est?'bg-indigo-600 text-white border-indigo-600 shadow':'bg-white text-slate-600 hover:bg-slate-100 border-slate-200'}`}>
                          {est}
                        </button>
                      ))}
                    </div>
                    <div className="flex gap-2 mb-6">
                      {[
                        {label:'WhatsApp',c:'green',action:()=>window.open(`https://wa.me/57${venDetail.customer_phone||''}`)},
                        {label:'Llamar',c:'blue',action:()=>window.open(`tel:${venDetail.customer_phone||''}`)},
                        {label:'Email',c:'slate',action:()=>window.open(`mailto:${venDetail.customer_email||''}`)},
                      ].map((b,i)=>(
                        <button key={i} onClick={b.action}
                          className={`flex-1 bg-${b.c}-50 text-${b.c}-700 hover:bg-${b.c}-100 py-2.5 rounded-xl font-bold flex justify-center items-center gap-2 text-sm border border-${b.c}-200 transition-colors`}>
                          {b.label}
                        </button>
                      ))}
                    </div>
                    <p className="text-xs font-black text-slate-400 uppercase mb-3">Historial</p>
                    <div className="relative border-l-2 border-slate-100 ml-4 pl-6 pb-4">
                      {(venDetail.actividades||[{created_at:venDetail.created_at,action:'CREATED',description:'Pedido de venta creado'}]).map((a:any,i:number)=>(
                        <div key={i} className="mb-5 relative">
                          <div className="absolute -left-[29px] top-1.5 w-4 h-4 rounded-full bg-white border-2 border-indigo-200 flex items-center justify-center">
                            <div className="w-2 h-2 bg-indigo-500 rounded-full"/>
                          </div>
                          <p className="text-xs text-slate-400 mb-0.5">{fDate(a.created_at)} {fTime(a.created_at)}</p>
                          <p className="font-bold text-slate-700 text-sm">{a.description||a.action}</p>
                        </div>
                      ))}
                    </div>
                  </div>
