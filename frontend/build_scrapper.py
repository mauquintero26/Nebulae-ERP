...
import { 
  ...
  ExternalLink, Tag, TrendingDown, Percent, Box, Plus
} from 'lucide-react';

const TARGET_STORES = [
  { id: 1, name: 'Amazon B2B', url: 'https://amazon.com/...', status: 'Active', lastSync: 'Hace 10 min', itemsFound: 124 },
  { id: 2, name: 'Distribuidor Mayorista', url: 'https://mayorista.com/...', status: 'Paused', lastSync: 'Hace 2 das', itemsFound: 45 },
...
const SCRAPED_PROMOS = [
  { id: 'P001', product: 'Monitor Dell 27"', store: 'Amazon B2B', originalPrice: 250, promoPrice: 190, discount: 24, matchSku: 'MON-DELL-27' },
  { id: 'P002', product: 'Silla Ergonmica Herman Miller', store: 'Distribuidor Mayorista', originalPrice: 1200, promoPrice: 850, discount: 29, matchSku: 'S-HM-01' },
  { id: 'P003', product: 'Teclado Mecnico Keychron', store: 'Competencia Directa', originalPrice: 100, promoPrice: 85, discount: 15, matchSku: 'TEC-K-M' },
  { id: 'P004', product: 'Webcam Logitech Brio', store: 'Amazon B2B', originalPrice: 180, promoPrice: 120, discount: 33, matchSku: 'CAM-LOG-B' },
];
    ...
    <div className="w-full bg-slate-50 min-h-max pb-12 animate-in fade-in custom-scrollbar">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-6 sticky top-0 z-30 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Motor de extraccin autnoma para monitoreo de precios, ofertas de competidores y distribuidores.</p>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/80 border-b border-slate-200 text-[11px] font-black text-slate-500 uppercase tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-center">Accin</th>
                  </tr>
                </thead>
          ...
          {activeTab === 'Fuentes' && (
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-black text-slate-800">URLs y Reglas de Extraccin</h3>
                <button className="bg-blue-50 text-blue-700 hover:bg-blue-100 px-4 py-2 rounded-lg text-xs font-bold transition-colors flex items-center gap-2">
                  <Plus size={14}/> Aadir Nueva Tienda
                </button>
              </div>
                  ...
                  <div key={store.id} className="border border-slate-200 rounded-xl p-4 flex flex-col">
                    ...
                    <div className="mt-auto pt-4 border-t border-slate-100 flex justify-between items-center">
                      ...
                      <div className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                        <Clock size={10}/> lt. Sync: {store.lastSync}
                      </div>
                    </div>
                  </div>
                ))}
