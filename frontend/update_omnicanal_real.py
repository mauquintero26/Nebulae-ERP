...
new_col4 = """{/* COLUMN 4: CRM + AI COPILOT */}
      <div className="bg-slate-50 border-l border-slate-200 flex flex-col relative overflow-hidden flex-shrink-0" style={{ width: '350px', minWidth: '300px', resize: 'horizontal' }}>
        
        {/* TOP: CRM PROFILE (Resizable Vertically) */}
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col bg-white border-b border-slate-300 relative" style={{ minHeight: '300px', resize: 'vertical', overflow: 'auto' }}>
          ...
          {/* Resize Handle Handle for UX */}
          <div className="h-1 w-full bg-slate-200 cursor-row-resize hover:bg-purple-300 transition-colors"></div>
        </div>

        {/* BOTTOM: NEBULAE IA (Expands to fill remaining space) */}
        <div className="flex-1 flex flex-col bg-gradient-to-b from-slate-50 to-indigo-50/50 min-h-[200px]">
...
with open('src/app/dashboard/asistente_omnicanal/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Omnicanal page REALLY updated")
