"""
test_fase4_migrations.py — Integridad de Migración fa4_001, Roundtrip y Seguridad (Fase 4).

Escenarios cubiertos:
1. Verificación de tablas, columnas, checks y precisión decimal de fa4_001 en erp_test:
   ...
   - Columnas nuevas en sale_orders y sale_order_lines_erp.
   - Check constraints de modalidad, propietario, estado y resoluciones.
   - Precisión NUMERIC(10, 2) / NUMERIC(14, 2).
2. Auditoría estática de seguridad:
   - fa4_001 NO debe contener GRANTs dirigidos a nebulae_test ni modificar permisos.
3. Ciclo de vida de migración (Roundtrip downgrade fa3_002 -> upgrade head):
   - Downgrade limpio a fa3_002 eliminando tablas, secuencias y columnas agregadas sin huérfanos.
   - Upgrade de regreso a head (fa4_001) restaurando la integridad completa.
...
class TestFase4Migrations:
    ...
    def test_fa4_001_tablas_columnas_checks_indices(self, db):
        """Verifica la existencia física de las 7 tablas de Fase 4, columnas agregadas y checks."""
        ...
        # 2. Nuevas columnas en sale_orders
        ...
        for col in [
            ...
            "cancellation_authorized_by", "cancelled_at"
        ]:
            assert col in so_cols, f"Columna {col} no encontrada en sale_orders"

        # 3. Nuevas columnas en sale_order_lines_erp
        sol_cols = {r[0] for r in db.execute(text("""
            SELECT column_name FROM information_schema.columns WHERE table_name = 'sale_order_lines_erp'
        """)).fetchall()}
        for col in [
            "customer_id", "tax_pct", "modalidad", "owner", "quantity_reserved",
            "quantity_delivered", "quantity_cancelled", "estado",
            "cost_unit_cop_snapshot", "price_unit_cop_snapshot", "updated_at"
        ]:
            assert col in sol_cols, f"Columna {col} no encontrada en sale_order_lines_erp"

        # 4. Validar check constraints principales
        constraints_sol = [r[0] for r in db.execute(text("""
            SELECT conname FROM pg_constraint WHERE conrelid = 'sale_order_lines_erp'::regclass
        """)).fetchall()]
        assert "chk_sol_modalidad" in constraints_sol
        assert "chk_sol_owner" in constraints_sol
        assert "chk_sol_estado" in constraints_sol
    ...
    def test_fa4_static_audit_no_test_role_grants(self):
        """Auditoría estática: fa4_001 no debe contener GRANTs hacia nebulae_test ni GRANT ALL."""
        mig_file = _BACKEND / "alembic" / "versions" / "fa4_001_sales_payments_delivery.py"
        assert mig_file.exists(), f"Archivo de migración no encontrado en {mig_file}"
        code = mig_file.read_text(encoding="utf-8")
        assert "nebulae_test" not in code, "VIOLACIÓN DE SEGURIDAD: fa4_001 contiene 'nebulae_test'"
        assert "grant all" not in code.lower(), "VIOLACIÓN DE SEGURIDAD: fa4_001 contiene 'GRANT ALL'"
    ...
    def test_fa4_roundtrip_downgrade_fa3_002_upgrade_head(self):
        ...
        downgrade fa3_002 -> upgrade head.
        ...
        # 2. Upgrade de regreso a head (fa4_001)
        up = subprocess.run(
        ...
        with eng.connect() as conn:
            ...
            assert v_up == "fa4_001", f"Versión esperada fa4_001, obtenida {v_up}"
